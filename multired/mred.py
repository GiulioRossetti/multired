...
__all__ = ["MultiplexRed", "Layer", "XlogxFit"]


class XlogxFit(object):
    ...
    def __getitem__(self, index):
        if index <= self.degree:
            ...
            print(
                "Error!!! Index %d is larger than the degree of the fitting polynomial (%d)"
                % (index, self.degree)
            )
            sys.exit(-1)
...
class Layer(object):
                ...
                with open(layerfile, "r") as lines:
                    for l in lines:
                        ...
                        if len(elems) > 2:  # A weight is specified
                            val = [
                                float(x) if "e" in x or "." in x else int(x)
                                for x in [elems[2]]
                            ][0]
    ...
    def make_matrices(self, N):
        self.N = N
        self.adj_matr = csr_matrix(
            (self._ww, (self._ii, self._jj)), shape=(self.N, self.N)
        )
...
class MultiplexRed(object):
    def __init__(self, multiplexfile, directed=None, fit_degree=10, verbose=False):
        ...
        self.poly = XlogxFit(self.fit_degree)
        self.verb = verbose
        self.cuts = None
        ...
        try:
            ...
            print("Unable to find/open file -- Exiting!!!")
            exit(-2)
    ...
    def compute_JSD_matrix_approx(self):
        ...
        for i in range(len(self.layers)):
            for j in range(i + 1, len(self.layers)):
                ...
                d = m_sigma.entropy_approx - 0.5 * (
                    li.entropy_approx + lj.entropy_approx
                )
                d = math.sqrt(d)
    ...
    def dump_JSD(self, force_compute=False):
        if self.JSD is None:
            if force_compute:
                ...
                print(
                    "Error!!! call to dump_JSD but JSD matrix has not been computed!!!"
                )
                sys.exit(1)

        # the len field does not exist in MultiplexRed
        # idx = 0
    ...
    def dump_JSD_approx(self, force_compute=False):
        if self.JSD_approx is None:
            if force_compute:
                ...
                print(
                    "Error!!! call to dump_JSD_approx but JSD approximate matrix has not been computed!!!"
                )
                sys.exit(1)
        idx = 0
    ...
    def draw_dendrogram(self, force=False):
        if not has_matplotlib:
            sys.stderr.write(
                "No matplotlib module found in draw_dendrogram...Exiting!!!\n"
            )
            sys.exit(3)
        if self.Z is None:
            if not force:
    ...
    def draw_dendrogram_approx(self, force=False):
        if not has_matplotlib:
            sys.stderr.write(
                "No matplotlib module found in draw_dendrogram_approx...Exiting!!!\n"
            )
            sys.exit(3)
        if self.Z_approx is None:
            if not force:
                sys.stderr.write(
                    "Please call reduce_approx() first or specify 'force=True'"
                )
            else:
                self.reduce_approx()
